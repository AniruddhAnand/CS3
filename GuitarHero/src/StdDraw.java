...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    ***************************************************************************/
     ...
     */
    public static void myLine(double x0, double y0, double x1, double y1) {
        offscreen.draw(new Line2D.Double(scaleX(x0), scaleY(y0), scaleX(x1), scaleY(y1)));
        draw();
    }
    public static void myLine(int n, double[] y) {
        GeneralPath path = new GeneralPath();
        path.moveTo((float) scaleX(0), (float) scaleY(y[0]));
        for (int i = 0; i < n; i++)
            path.lineTo((float) scaleX(i), (float) scaleY(y[i]));
        offscreen.draw(path);
        draw();
    }
