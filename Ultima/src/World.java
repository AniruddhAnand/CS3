...
public class World
{
	...
	private Monster boss; // Holds the monster objects
	...
	public World(String filename)
	{
		...
		boss = new Monster(this,"OR",avatar.getX(),avatar.getY()+1,100,15,500);
	}
	...
	{
		if(currentX>=tiles.length||currentY>=tiles[0].length||currentX<0||currentY<0) {
			return 0;
		}
		if(distance(x,currentX,y,currentY)>r) return 0;
		int count = 0;
		tiles[currentX][currentY].setLit(true);
		if(tiles[currentX][currentY].isOpaque()){
			return 1;
		}
		if(currentX+1<tiles.length&&!tiles[currentX+1][currentY].getLit()){
			count+=1+light(x,y,currentX+1,currentY,r);
		}
		if(currentX-1>-1&&!tiles[currentX-1][currentY].getLit()){
			count+=1+light(x,y,currentX-1,currentY,r);
		}
		if(currentY+1<tiles[currentX].length&&!tiles[currentX][currentY+1].getLit()){
			count+=1+light(x,y,currentX,currentY+1,r);
		}
		if(currentY-1>-1&&!tiles[currentX][currentY-1].getLit()){
			count+=1+light(x,y,currentX,currentY-1,r);
		}
		return count;
	}
