...
public class Monster implements Runnable
{
	...
	public void draw()
	{
		...
		switch (type)
		{
		case SKELETON:	StdDraw.picture(drawX, drawY, "mob1.jpg",32,32); 	break;
		case ORC:		StdDraw.picture(drawX, drawY, "mob2.jpg",32,32); 		break;
		case BAT:		StdDraw.picture(drawX, drawY, "mob3.jpg",32,32);     	break;
		case SLIME:		StdDraw.picture(drawX, drawY, "mob4.jpg",32,32); 	break;
		default: 														break;
		}
	...
	public void run() {
		while (hp > 0) {
			int x = ((int) (Math.random() * 3) - 1)+this.x;
			int y = ((int) (Math.random() * 3) - 1)+this.y;
		...
		if(Math.random()>.9) {
			...
			world.getAvatar().upDamage(type == MonsterType.ORC ? 3 : type == MonsterType.SKELETON ? 2 : 1);
		}
	}
}
